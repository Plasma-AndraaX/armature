...
### Added
- **New lesson: a plugin install made "just for one run" outlives the run.** Installing Armature at **user** scope to drive a single isolated-worktree run (2026-09-09) left the kit's own repo with two competing sources for `/armature:*` — the cached snapshot, active everywhere, and the `--plugin-dir ./plugin` the whole dogfooding doctrine rests on. The existing lesson only warned against installing the plugin *in* this repo; it said nothing about an install made elsewhere, at user scope, landing here by default. Recorded in `docs/lessons-technical.md` with what was actually established (user scope is global; `claude plugin list` does not show a `--plugin-dir` plugin, so seeing no conflict proves nothing) and what was *not* (whether a project-scoped `disable` spares the `--plugin-dir` plugin — untestable in `-p`, left open rather than guessed).
