...
- If `<target>/CLAUDE.md` already exists, stop and ask the user how to proceed: **overwrite**, **merge**, or **abort**. Never silently overwrite existing project docs. Note on `merge`: there's nothing to diff yet at this point — Phase 0 only captures *which* of the three paths to take. Run Phases 1-5 as normal to produce the candidate content, then, right before Phase 6 commits, show a diff of what would actually change per file and apply only what the user confirms (see Phase 6). **Where the candidate content goes in the meantime**: a file whose target path is free is written normally; a file that would land on an existing one is written to `<target>/.bootstrap-candidate/<same relative path>` instead — never over the existing file, which must still be intact when the diff is shown. Whatever the user declines stays there for them to look at, and `.bootstrap-candidate/` is deleted once they've confirmed they're done with it. Don't rely on git to recover an overwritten file: Phase 6 explicitly handles targets that aren't repos yet.
...
- Grep for `TODO`/`FIXME`/`XXX` comments across source files. **Count only real ones**: a marker inside a *source file*, left by someone who meant to come back to it. Exclude markers living in documentation or prose that merely *discusses* them, and exclude any template/scaffold tree the project itself ships (a placeholder deliberately addressed to the project's own users is not a TODO of this codebase). If what's left is a non-trivial number, count them and keep a few representative samples for Phase 4 — don't triage or import them yet. If the grep's raw count is high but nearly all of it is excluded by the rule above, don't ask the Phase 4 question at all.
...
**Read the target before proposing anything.** The phase above is written for the common case — a project whose `docs/adr/`, `docs/backlog/` and `docs/lessons-technical.md` don't exist yet or hold nothing but their template entry. On a project that already has some (Phase 0's `merge` path, or a repo with docs of its own), the harvest is a **delta**: read what's already there first, and drop every candidate the target already records — a git history is precisely the raw material of the ADRs and lessons someone already wrote from it. Proposing an item that duplicates an existing one is worse than proposing nothing.

Two or three lessons and a handful of backlog items is a normal harvest. A dozen lessons means the filter isn't doing its job. On a target that already documents itself, zero is a perfectly good answer — say so and move on.
...
**Never overwrite a pre-existing file, whatever the mapping says.** The mapping below describes where each template file *goes*; it assumes the slot is free. Whenever the target already has a file at that path, the mapping does not authorize replacing it — fall back to the `merge` handling described in Phase 0 (write your version to `<target>/.bootstrap-candidate/<same relative path>` and show the diff before anything is applied). Three cases carry their own explicit rule below (`.gitignore` appends, `.claude/settings.json` merges, `claude.sh` never overwritten); every other file follows this general one.
...
- `templates/claude.sh` → `<target>/claude.sh`, then `chmod +x` it. **If the target already has a `claude.sh`, do not overwrite it** — a project may have its own launcher with different duties (this kit's own repo does: a dogfooding wrapper). Treat it like the two other pre-existing-file cases below: show what the template's version adds, and let the user merge or decline.
...
- `docs/lessons-technical.md` → one entry per confirmed lesson, newest first, in the file's own format (bold actionable title, 2-3 paragraphs, `_Captured YYYY-MM-DD._`). Delete the file's `[Template — copy this shape for a new entry]` entry once at least one real entry is in — it exists only to show the shape on an empty file (a target that already has real entries has no such placeholder: leave its content alone).
